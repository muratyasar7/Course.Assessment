...
namespace Platform.Analytics.Infrastructure.Outbox;
...
internal sealed class ProcessOutboxMessagesJobSetup : IConfigureOptions<QuartzOptions>
{
    ...
    {
        const string jobName = nameof(ProcessQueueMessagesJob);

        options
            .AddJob<ProcessQueueMessagesJob>(configure => configure.WithIdentity(jobName))
